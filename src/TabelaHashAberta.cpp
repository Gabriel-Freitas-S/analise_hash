...
size_t TabelaHashAberta::sondagemLinear(size_t indiceInicial, int valor, bool paraInsercao) const {
    size_t tentativas = 0;
...
std::optional<int> TabelaHashAberta::remover(int valor, TipoHash tipo) {
    size_t indiceInicial = (tipo == TipoHash::DIVISAO) 
        ? calcularHashDivisao(valor) 
        : calcularHashMultiplicacao(valor);
...
TabelaHashAberta::EstatisticasSondagem TabelaHashAberta::analisarSondagem() const {
...
std::ostream& operator<<(std::ostream& os, TabelaHashAberta::TipoHash tipo) {
    switch (tipo) {
        case TabelaHashAberta::TipoHash::DIVISAO:
            os << "Divisão";
            break;
        case TabelaHashAberta::TipoHash::MULTIPLICACAO:
            os << "Multiplicação";
            break;
    }
    return os;
