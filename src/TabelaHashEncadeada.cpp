...
bool TabelaHashEncadeada::remover(int valor, TipoHash tipo) {
    size_t indice = (tipo == TipoHash::DIVISAO) 
        ? calcularHashDivisao(valor) 
        : calcularHashMultiplicacao(valor);
...
TabelaHashEncadeada::EstatisticasDistribuicao TabelaHashEncadeada::obterEstatisticas() const {
...
std::ostream& operator<<(std::ostream& os, TabelaHashEncadeada::TipoHash tipo) {
    switch (tipo) {
        case TabelaHashEncadeada::TipoHash::DIVISAO:
            os << "Divisão";
            break;
        case TabelaHashEncadeada::TipoHash::MULTIPLICACAO:
            os << "Multiplicação";
            break;
    }
    return os;
